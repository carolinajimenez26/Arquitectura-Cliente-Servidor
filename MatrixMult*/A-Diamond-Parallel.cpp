...
void dot(SafeGraph &m1, SafeGraph &m2, SafeGraph &res, int &min, int v, const int nodes) {
  int value;
...
int parallelMult2(int p, SafeGraph &original, SafeGraph &current, SafeGraph &res) {
  const int nodes = p;
  int min = INF, chunks = 0, cpu = 0, index = 0;
  bool flag = false;
  dbg(nodes);
  ...
  while (p > 0) {
    ...
    for (; i < chunks; i++) {
      ...
      for (int k = 0; k < cpu; k++) {
        ...
        index = k + cpu*i;
        dbg(index);
        if (p % 2 == 0) ts.push_back(thread(dot, ref(current), ref(current), ref(res), ref(min), index, nodes));
        else ts.push_back(thread(dot, ref(original), ref(current), ref(res), ref(min), index, nodes));
      }
...
int parallelMult(int p, SafeGraph &original, SafeGraph &current, SafeGraph &res) {
  ...
  while (p > 0) {
    ...
    for (auto& v : current.m) { // rows m1
      ...
      [&current, &res, &min, &v, nodes]() { dot(ref(current), ref(current), ref(res), ref(min), v.first, nodes); });
      else pool.submit( //
      [&original, &current, &res, &min, &v, nodes]() { dot(ref(original), ref(current), ref(res), ref(min), v.first, nodes); });
    }
